...
public class QuestionHandler(AppDbContext context) : IQuestionHandler
{
    ...
    public async Task<Response<Question?>> DeleteAsync(DeleteQuestionRequest request)
    {
        ...
        {
                var question =
                    await context.Questions.FirstOrDefaultAsync(x => x.Id == request.Id && x.User == request.User);
    ...
    public async Task<Response<Question?>> UpdateAsync(UpdateQuestionRequest request)
    {
        try
        {
                var question = await context.Questions.FirstOrDefaultAsync(x => x.Id == request.Id && x.User == request.User);
                ...
                question.UpdatedBy = request.User;
